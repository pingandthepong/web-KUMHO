...
$(".tab").on("mouseenter focus", function () {
  ...
  $(".master__main-txt").hide();
  $(".master__inner").css({
    background: `url("./mainimages/${tabInfo.bgUrl}") no-repeat 0 0`,
    transition: "all .1s",
  });

  $(".tab-title").text(tabInfo.title);
  $(".tab-desc").text(tabInfo.desc);

  $(".master__tab-conlist")
    .css({ opacity: 0, left: 0 })
    .stop()
    .animate({ opacity: 1, left: "150px" });
});

$(".tab").on("mouseleave blur", function () {
  $(".tab").removeClass("active");

  $(".master__main-txt").show();
  $(".master__inner").css({
